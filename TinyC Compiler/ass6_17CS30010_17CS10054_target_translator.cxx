#include "ass6_17CS30010_17CS10054_translator.h"
#include "y.tab.h"
	...
	while(i<next_instr)
	{
		switch(glob_quad.arr[i].op)
		{
			...
			{	
				...
				{
					count++;
					mp_set[atoi(glob_quad.arr[i].result.c_str())]=count-1;
				}
			}
		}
		i++;
...
void symtab::global_variables(FILE *fp)
{
	...
	{
		if(symbol_tab[i]->name[0]!='t' &&symbol_tab[i]->tp_n&&symbol_tab[i]->var_type!="func")
		{
			if(tp_char==symbol_tab[i]->tp_n->basetp)
			{
				cs.push_back(symbol_tab[i]->name);
				if(!(symbol_tab[i]->isInitialized))
				{
					count++;
			...
			if(tp_int==symbol_tab[i]->tp_n->basetp)
			{
				...
				else if(symbol_tab[i]->isInitialized==true)
				{
					fprintf(fp,"\n\t.globl\t%s",symbol_tab[i]->name.c_str());
					fprintf(fp,"\n\t.data");
					fprintf(fp,"\n\t.align 4");
...
void symtab::assign_offset()
{
    ...
    while(i<(symbol_tab).size())
    {
        ...
        if(!(symbol_tab[i]->isdone)&&symbol_tab[i]->var_type!="return"&&symbol_tab[i]->var_type!="param")
        {
			symbol_tab[i]->isdone=true;
        	symbol_tab[i]->offset=curr_offset-symbol_tab[i]->size;
        	curr_offset=symbol_tab[i]->offset;
        }
        else if(symbol_tab[i]->isdone==false&&symbol_tab[i]->var_type=="param")
        {
        	if(symbol_tab[i]->tp_n)
        	{
				if(symbol_tab[i]->tp_n->basetp==tp_arr){
					if(symbol_tab[i]->tp_n->size==-1)
					{
						symbol_tab[i]->isptrarr=true;
					}
...
void symtab::func_prologue(FILE *fp,int count)
{
	int k=0;
	fprintf(fp,"\n\t.globl\t%s",name.c_str());
	fprintf(fp,"\n\t.type\t%s, @function",name.c_str());
...
int symtab::func_restore(FILE* fp,int a[], int size) 
{ 
    int count=0,i=symbol_tab.size()-1,k=0;
	string regname;
	while(i>=0)
	{
	    if(symbol_tab[i]->ispresent==false){
...
int symtab::function_call(FILE *fp)
{
	...
	fprintf(fp,"\n\tpushq %%rbp");
	int count=0;
	while(params_stack.size() && count<6)
	{
...
void symtab::function_restore(FILE *fp)
{
	...
	while(i>=0)
	{
	    ...
	    if(symbol_tab[i]->offset<0 && symbol_tab[i]->var_type=="param")
	    {
		    if(symbol_tab[i]->tp_n->basetp == tp_char){
	            regname = assign_reg(tp_char,count);
	            fprintf(fp,"\n\tmovb\t%%%s, %d(%%rbp)",regname.c_str(),symbol_tab[i]->offset);
